          ...
          <FadeIn className="container px-4 md:px-6 relative z-10 mx-auto text-center">
            ...
            <div className="space-y-3 max-w-4xl mx-auto">
              <h1 className="text-4xl font-extrabold tracking-tight sm:text-5xl md:text-6xl lg:text-7xl/none">
                {t('landing.heroTitle1')} <span className="text-primary underline decoration-4 underline-offset-8 decoration-primary/30">{t('landing.heroTitle2')}</span>
              </h1>
              <p className="mx-auto max-w-[700px] text-muted-foreground md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed mt-4">
                {t('landing.heroDesc1')} <span className="font-semibold text-primary">KSP Crime Database</span>{t('landing.heroDesc2')}
              </p>
            </div>
        ...
        <section className="w-full py-24 relative bg-background border-t border-border/40 overflow-hidden">
          <div className="container px-4 md:px-6 mx-auto text-center relative z-10">
            <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl">
              {t('landing.systemTitle1') || 'The Ultimate'} <span className="text-primary underline decoration-4 underline-offset-8 decoration-primary/30">{t('landing.systemTitle2') || 'Intelligence System'}</span>{t('landing.systemTitle3') || ', Tailored Exclusively for CrimeIntel'}
            </h2>
            <p className="mx-auto max-w-[800px] text-muted-foreground md:text-lg mt-6 mb-20">
              {t('landing.systemDesc1') || 'CrimeIntel offers'} <span className="font-semibold text-primary">{t('landing.systemDesc2') || 'real-time crime mapping'}</span>{t('landing.systemDesc3') || ', suspect profiling, intelligence graphs, and an automated case file system that integrates deeply with existing law enforcement infrastructure.'}
            </p>
            ...
            <div className="relative w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-center gap-y-12 md:gap-y-0 md:gap-x-16 min-h-[300px]">
               ...
               <div className="flex flex-col gap-y-6 relative w-full md:w-auto items-center md:items-end">
                 ...
                 <FlowCard icon={<Map className="w-5 h-5"/>} text={t('landing.flow.mapping') || "Real-time Crime Mapping"} side="left" />
                 <FlowCard icon={<Users className="w-5 h-5"/>} text={t('landing.flow.profiling') || "Suspect Profiling Database"} side="left" />
                 <FlowCard icon={<FolderOpen className="w-5 h-5"/>} text={t('landing.flow.caseFiles') || "Automated Case Files"} side="left" />
               </div>
               ...
               <div className="flex flex-col gap-y-6 relative w-full md:w-auto items-center md:items-start">
                 ...
                 <FlowCard icon={<Radio className="w-5 h-5"/>} text={t('landing.flow.cad') || "Live Unit Dispatch (CAD)"} side="right" />
                 <FlowCard icon={<Network className="w-5 h-5"/>} text={t('landing.flow.network') || "Network Intelligence Graphs"} side="right" />
                 <FlowCard icon={<BrainCircuit className="w-5 h-5"/>} text={t('landing.flow.predictive') || "Predictive Crime Analytics"} side="right" />
               </div>
            </div>
          ...
          <FadeIn className="container px-4 md:px-6 mx-auto">
            <div className="text-center pb-12">
              <h2 className="text-2xl md:text-[28px] text-zinc-600 max-w-4xl mx-auto leading-relaxed font-normal">
                {t('landing.previewDesc') || 'Access comprehensive intelligence dashboards, cross-reference suspect data instantly, and export detailed case reports in seconds. Turn raw data into actionable police intelligence.'}
              </h2>
               ...
               <div className="flex flex-col border-b lg:border-b-0 lg:border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
                ...
                <div className="p-6 md:p-8 lg:pl-10 border-l-[4px] border-zinc-900 dark:border-white bg-zinc-50 dark:bg-zinc-900/20 cursor-pointer">
                   <h3 className="text-lg md:text-xl font-bold flex items-center mb-2">
                     <Search className="mr-3 h-5 w-5 text-zinc-700 dark:text-zinc-300" /> {t('landing.tabs.search') || 'Search → Cross-Reference'}
                   </h3>
                   <p className="text-muted-foreground text-[14px] leading-relaxed max-w-sm">
                     {t('landing.tabs.searchDesc') || 'Instantly query databases, cross-reference suspect aliases, and identify hidden network connections across multiple districts.'}
                   </p>
                </div>
                ...
                <div className="p-6 md:p-8 lg:pl-10 border-l-[4px] border-transparent hover:bg-zinc-50/50 cursor-pointer transition-colors border-t border-b border-zinc-200 dark:border-zinc-800">
                   <h3 className="text-lg md:text-xl font-bold flex items-center mb-2">
                     <Copy className="mr-3 h-5 w-5 text-zinc-700 dark:text-zinc-300" /> {t('landing.tabs.export') || 'Export Automated Reports'}
                   </h3>
                   <p className="text-muted-foreground text-[14px] leading-relaxed max-w-sm">
                     {t('landing.tabs.exportDesc') || 'Generate comprehensive FIR summaries, export intelligence dossiers, and prepare court-ready documents with a single click.'}
                   </p>
                </div>
                ...
                <div className="p-6 md:p-8 lg:pl-10 border-l-[4px] border-transparent hover:bg-zinc-50/50 cursor-pointer transition-colors">
                   <h3 className="text-lg md:text-xl font-bold flex items-center mb-2">
                     <Share2 className="mr-3 h-5 w-5 text-zinc-700 dark:text-zinc-300" /> {t('landing.tabs.secure') || 'Secure Case Management'}
                   </h3>
                   <p className="text-muted-foreground text-[14px] leading-relaxed max-w-sm">
                     {t('landing.tabs.secureDesc') || 'Manage active investigations securely, save digital evidence, and collaborate in real-time with other stations.'}
                   </p>
                </div>
               </div>
            ...
            <div className="bg-zinc-50/80 dark:bg-zinc-900/30 flex flex-col items-center justify-center p-8 md:p-12 relative overflow-hidden min-h-[500px]">
               ...
               <div className="mt-8 z-10 relative">
                  <div className="text-[10px] text-zinc-500 font-semibold tracking-widest uppercase bg-white border border-zinc-200 px-5 py-2 rounded-full shadow-sm">
                    {t('landing.previewBadge') || 'LIVE AI GENERATION PREVIEW'}
                  </div>
               </div>
             </div>
            </div>
          ...
          <div className="container px-4 md:px-6 mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_2fr] gap-8 md:gap-12 pb-8 md:pb-10 border-b border-dashed border-zinc-200 dark:border-zinc-800">
               <div className="flex flex-col justify-center space-y-4">
                  <h3 className="text-[22px] font-bold tracking-tight">{t('landing.products.dashboard') || 'Central Intelligence Dashboard'}</h3>
                  <p className="text-muted-foreground text-[17px] leading-relaxed">
                    {t('landing.products.dashboardDesc') || 'Real-time monitoring and analytics for police superintendents, inspectors, and intelligence officers.'}
                  </p>
                  <ul className="space-y-3 text-muted-foreground mt-2 text-[15px]">
                     <li className="flex items-start"><CheckCircle2 className="mr-3 h-5 w-5 text-primary shrink-0" /> {t('landing.products.dashBullet1') || 'Seamless integration with state CCTNS and Zoho databases.'}</li>
               ...
               <div className="grid grid-cols-2 md:grid-cols-3 gap-4 lg:gap-6 items-center">
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/crime_heatmap_ui_1784728718764.png" alt="Crime Heatmap" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/police_cad_dashboard_ui_1784723433845.png" alt="CAD Dashboard" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/unit_tracker_ui_1784728761379.png" alt="Unit Tracker Map" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/dashboard-ui.png" alt="Dashboard UI" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/analytics-ui.png" alt="Analytics UI" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/crime_case_file_ui_1784723422475.png" alt="Case File" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
               </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_2fr] gap-8 md:gap-12 pt-8 md:pt-10">
               <div className="flex flex-col justify-center space-y-4">
                  <h3 className="text-[22px] font-bold tracking-tight">{t('landing.products.network') || 'Network Analysis & Graphing'}</h3>
                  <p className="text-muted-foreground text-[17px] leading-relaxed">
                    {t('landing.products.networkDesc') || 'Powerful entity-relationship graphs to visualize connections between suspects, vehicles, properties, and criminal organizations.'}
                  </p>
                  <ul className="space-y-3 text-muted-foreground mt-2 text-[15px]">
                     <li className="flex items-start"><CheckCircle2 className="mr-3 h-5 w-5 text-primary shrink-0" /> {t('landing.products.netBullet1') || 'Detect money laundering rings and organized crime syndicates.'}</li>
               ...
               <div className="grid grid-cols-2 md:grid-cols-3 gap-4 lg:gap-6 items-center">
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/intelligence_graph_ui_1784723448238.png" alt="Intelligence Graph" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/facial_recognition_ui_1784728734412.png" alt="Facial Recognition" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/crime_case_file_ui_1784723422475.png" alt="Case File UI" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/network-ui.png" alt="Network UI" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl overflow-hidden border border-zinc-200/80 dark:border-zinc-800 aspect-video relative group shadow-sm transition-transform hover:scale-[1.02]">
                     <Image src="/images/crime_heatmap_ui_1784728718764.png" alt="Crime Heatmap" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
          ...
          <div className="container px-4 md:px-6 mx-auto text-center max-w-4xl">
            ...
            <div className="space-y-4">
              <p className="text-[15px] text-muted-foreground">
                <strong className="text-foreground font-semibold">{t('landing.security.bullet1') || 'Strictly Audited Access:'}</strong> {t('landing.security.bullet1Desc') || 'Every search, query, and export is logged and monitored for compliance and operational integrity.'}
              </p>
              <p className="text-[15px] text-muted-foreground flex items-center justify-center gap-2">
                <span className="text-zinc-400">✓</span> {t('landing.security.bullet2') || 'Regular security patches and intelligence module updates.'}
              </p>
            </div>
          </div>
        </section>
